...
import type { Character, Move, Relic } from '@anime-showdown/shared-types';
...
const relics: Relic[] = JSON.parse(readFileSync(resolve(dataDir, 'relics.json'), 'utf-8'));
...
// GET /api/roster/relics/all — all relics
router.get('/relics/all', (_req, res) => {
  res.json(relics);
});
