        ...
        {activeBattleTab && (
          <div style={{ flex: 1, minHeight: 0, width: '100%', overflow: 'hidden', padding: '4px 10px', display: 'flex', flexDirection: 'column' }}>
            ...
            {activeBattleTab.winner && (
              <div style={{ position: 'absolute', inset: 0, zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(4px)' }}>
                <div style={{ width: 450, padding: 32, borderRadius: 12, border: '2px solid var(--border-strong)', background: 'var(--card-bg)', boxShadow: '0 10px 40px var(--shadow-color)', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
                  <FighterLogo
                    id={activeBattleTab.winner === 'draw' ? 'handshake' : activeBattleTab.winner === activeBattleTab.yourKey ? 'trophy' : 'skull'}
                    size={64}
                    color="var(--text-primary)"
                  />
                  <h2 style={{ fontSize: '2rem', margin: '16px 0 6px', fontWeight: 900, color: 'var(--text-primary)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  ...
                  <p style={{ marginBottom: 24, color: 'var(--text-secondary)', fontWeight: 600 }}>
                  ...
                  <button className="btn btn-primary" style={{ width: '100%', height: 42, fontSize: '0.95rem', fontWeight: 900, textTransform: 'uppercase' }} onClick={() => { closeBattleTab(activeBattleTab.battleId); setActiveTab('lobby'); }}>
                    Return to Lobby
                  </button>
                </div>
              </div>
